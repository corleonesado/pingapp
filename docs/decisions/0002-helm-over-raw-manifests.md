...
- **Umbrella chart with subcharts for app + monitoring** — premature: the Prometheus stack is installed from its own upstream chart, not as a subchart. A flat chart keeps `helm lint` clean.
...
- **Positive:** `helm upgrade --install -f values-{dev,prod}.yaml` is one command per environment.
- **Positive:** `helm history` and `helm rollback` give a first-class rollout audit trail, which is the evidence the brief asks for.
- **Positive:** Conventional `app.kubernetes.io/*` labels mean the `ServiceMonitor` selector works without label gymnastics.
- **Positive:** CI can override `image.tag` with `--set image.tag=$SHA` without touching files in git.
- **Trade-off accepted:** Helm's templating uses Go text/template, which is whitespace-sensitive and easy to write subtle bugs in. Mitigated by `helm lint` and `helm template -f values-dev.yaml | kubeconform` in CI.
...
- Trimmed from the `helm create` skeleton: `templates/tests/`, `serviceaccount.yaml` (chart runs as the cluster default, locked down by `securityContext`), and the autoscaling/HPA stub.
